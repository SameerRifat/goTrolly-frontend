...
import { clear_errors, deleteUsers, delete_users_reset, getAllUsers } from '../../features/user/userSlice';
...
const UsersList = () => {
  ...
  const { getAllUsersLoading, allUsers, getAllUsersError, deleteUsersLoading, isUsersDeleted, deleteUsersError } = useSelector((state) => state.user)
  ...
  const columns = [
        ...
        return (
          <>
            <div className='flex items-center gap-4'>
              <Avatar className='' sx={{ bgcolor: params.row.avatarBggcolor, color: params.row.avatarBggcolor === 'yellow' ? '#000' : '#fff', textTransform: 'uppercase', fontSize: '15px', width: '37px', height: '37px' }}>
                {params.row.name
    ...
    //     return (
    //       <>
    //         <NavLink to={`/admin/user/${id}`} onClick={(event) => event.stopPropagation()}>
  ...
  const avatarColors = ['yellow', 'indigo', 'blue', 'green', 'orange', 'tomato'];
  const rows = []
  allUsers && allUsers.forEach((item, index) => {
    rows.push({
      id: item._id,
      name: item.name,
      email: item.email,
      role: item.role,
      date: item.createdAt,
      avatarBggcolor: avatarColors[index % avatarColors.length],
    })
  });
  ...
  useEffect(() => {
    if (isUsersDeleted) {
      ...
      setSelectedRows()
      dispatch(delete_users_reset())
      setOpen(false)
    }
    if (deleteUsersError) {
      toast.error(deleteUsersError, {
        position: "bottom-center",
        autoClose: 2000,
        hideProgressBar: false,
        closeOnClick: true,
  ...
  }, [dispatch, isUsersDeleted, deleteUsersError])

  return (
    <>
      <MetaData title="All Orders - Admin" />
      <h2 className='mb-4 font-medium text-2xl text-white'>Users</h2>
      {getAllUsersLoading ? <Loader /> : (
        allUsers.length > 0 ? (
          <Box
            sx={{
            ...
            <Box display='flex' alignItems='center' gap='20px' p='10px'>
              ...
              <Tooltip title="Update Status">
                <button className='flex items-center gap-1'
                  ...
                  onClick={roleToggle}
                >
                  <TuneOutlinedIcon fontSize='small' style={{ color: 'white' }} />
            ...
            <DataGrid
              ...
              getCellClassName={(params) => {
                if (params.field === 'role') {
                  return params.value === 'admin' ? 'text-green' : 'text-white';
                } else {
                  return 'text-white'
                }
              }}
      ...
      <UpdateRoleDialog modal={roleDialogOpen} toggle={roleToggle} selectedRowData={selectedRowsData} />
      ...
      >
        ...
        <DialogActions>
          ...
          <LoadingButton loading={deleteUsersLoading} variant="contained"
            onClick={() => dispatch(deleteUsers(selectedRows))}
            type='submit'
            sx={{
